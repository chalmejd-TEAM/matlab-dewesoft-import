...
% % close all figures
% % close all;
function list = dw_data_reader(fileName)
...
version = calllib(DWDataLibFile, 'DWGetVersion');
...
% Call the close file function in case another file was open. This doesn't matter much in a script but matters when you
% are going to manage functions and classes that read data. Calling close file function ensures no error with little
% time cost.
...
[DWStatus,~,info_data] = calllib(DWDataLibFile, 'DWOpenDataFile', FILE, struct());
statusCheck(DWStatus);
...
if event_count > 0
    ...
    event_list(1:event_count) = struct(libstruct('DWEvent'));
    ...
    for iEventIndex = 1:event_count
        ...
        [DWStatus,~,~,event_text] = calllib(DWDataLibFile, 'DWGetEventListItem', iEventIndex-1, event_type, time_stamp, empty_string, MAX_EVENT_TEXT_CHARACTERS);
        statusCheck(DWStatus);
...
if channel_count > 0
    ...
    channel_list(1:channel_count) = struct(libstruct('DWChannel'));
    ...
    for iChannel = 1:channel_count
        ...
        [DWStatus,~,name,unit,description] = calllib(DWDataLibFile, 'DWGetChannelListItem', iChannel-1, index, empty_string, empty_string, empty_string, color, array_size, MAX_CHAR_SIZE);
        statusCheck(DWStatus);

        % get number of samples for the current channel
        sample_count = calllib(DWDataLibFile, 'DWGetScaledSamplesCount', index.Value);
        ...
        if sample_count > 0
            ...
            DWStatus = calllib(DWDataLibFile, 'DWGetScaledSamples', index.Value, offset_position, sample_count, data, time);
            statusCheck(DWStatus);

            % save the data and time
            if array_size.Value > 1
                % reshape if its an array
                channel_list(iChannel).data = reshape(data.Value, sample_count, array_size.Value);
            else
        ...
        DWStatus = calllib(DWDataLibFile, 'DWGetReducedValuesCount', index.Value, sample_count_reduced, block_size_reduced);
        statusCheck(DWStatus);
        ...
        if sample_count_reduced.Value > 0
            ...
            DWStatus = calllib(DWDataLibFile, 'DWGetReducedAveValues', index.Value, offset_position_reduced, sample_count_reduced.Value, data_reduced, time_reduced);
            statusCheck(DWStatus);
...
header_count = calllib(DWDataLibFile, 'DWGetHeaderEntryCount');
...
if header_count > 0
    ...
    for iHeaderIndex = 1:header_count
        % strings are a bit different. Passing libpointer to a character array does not get updated. Therefore, we just
        % use the left hand side to pass out the character arrays
        [DWStatus, ~, name, unit, description] = calllib(DWDataLibFile, 'DWGetHeaderEntryListItem', iHeaderIndex-1, index, empty_string, empty_string, empty_string, color, array_size, MAX_CHAR_SIZE);
        satusCheck(DWStatus);

        % strings are a bit different. Passing libpointer to a character array does not get updated. Therefore, we just
        % use the left hand side to pass out the character arrays
        [DWStatus, header_info] = calllib(DWDataLibFile, DWGetHeaderEntryText, index.Value, empty_string2, MAX_HEADER_CHAR_SIZE);
...
list = channel_list(:,["name" "unit" "time" "data"]);
end
